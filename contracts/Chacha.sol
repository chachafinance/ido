...
contract Purcase is Ownable{
    ...
    constructor(address teamwallet_, address staks_,address chacha_,address token_)  {
        teamWallet = teamwallet_;
        staks = staks_;//质押合约销毁
        ...
        uint date = endTime;
        _thawInfoList.push(thawInfo(date + 60, 500));
        _thawInfoList.push(thawInfo(date + 90, 1583));
        _thawInfoList.push(thawInfo(date + 120, 1583));
        _thawInfoList.push(thawInfo(date + 150, 1583));
        _thawInfoList.push(thawInfo(date + 180, 1583));
        _thawInfoList.push(thawInfo(date + 210, 1583));
        _thawInfoList.push(thawInfo(date + 240, 1585));

        // for(uint8 i = 1; i < 5; i++) {
        //     vips.push(Vip({level:i ,start_num:i * 1000e18,end_num:0}));
        //     isVip[i] = true;
        //     vipList[i] = i * 1000e18;
        // }

        // burnsList.push(Burns({proportion:97,num:1000e18}));
        // burnsList.push(Burns({proportion:92,num:2000e18}));
        // burnsList.push(Burns({proportion:87,num:3000e18}));
        // burnsList.push(Burns({proportion:82,num:4000e18}));
    }
    modifier checkEnd() {
    ...
    uint256 baseProportion = 10000;
    uint256 public hasTokenCount;//进度数量
    uint256 public price = 40000; //ETH : CHACHA = 1: 40000
    uint256 public startTime = 1682329200;//开始时间
    uint256 public endTime =   1682332800;//申购结束
    uint256 public minAmount=2*1e16;//最小数量
    uint256 private _maxAmount = 10000*1e18;//最大数量
    uint256 public maxTokenCount = 10000*1e18;//进度额（最大额度数量）
    uint256 public target = 10*1e18;//预计值(目标数量)
    // uint256 public endDays=20;//释放之后时间（开始时间后(s)）
    // uint256 public time_days=30;//释放间隔时间
    // uint8 public number=3;//分隔数量（释放次数）
    ...
    function maxAmount(address addr) public view returns(uint256 num) {
       num = vipList[vip_level(addr)];
       if (num < _maxAmount) num = _maxAmount;
    }
    ...
    //投资
    function stake(uint256 amount) public payable checkEnd(){
        ...
        require(usdtMaxAmount <= maxAmount(msg.sender),"is not maxAmount");
       //销毁数量
        uint actualNumber;
        if(burnProportion(msg.sender) > 0){
            actualNumber = amount.mul(burnProportion(msg.sender)).div(freedBase);
        }else{
            actualNumber = amount;
        }
        require(msg.value >= actualNumber, 'Insufficient payment amount');
        payable(teamWallet).call{value:actualNumber}("");
        if (msg.value > actualNumber) {
            payable(msg.sender).call{value:msg.value - actualNumber}("");
        }
    ...
    function vip_level(address addr) view public returns (uint8){
        uint8 level = 0;
         for(uint256 i=0;i<vips.length;i++){
             if(_balances[addr]>= vips[i].start_num){
                  level = vips[i].level;
             }else if(_balances[addr]>= vips[i].start_num && vips[i].end_num == 0){
                  level = vips[i].level;
             }
        }
         return level;
    ...
    function editVip(thawInfo[] calldata thawInfoLis_) external onlyOwner{
        _thawInfoList = thawInfoLis_;
    }
    ...
    function earned(address addr) public view returns(uint256 amount){
        uint256 proportion = _getThawProportion(block.timestamp);
        Player storage user = _plyr[addr];
        amount = user.amount.mul(proportion).div(baseProportion);
    ...
    function getTakeOutNumber(address addr) public view returns (uint256){
        uint256 num = 0;
         for(uint256 i=0;i<ordersVip[addr].length;i++){
             if(block.timestamp >= ordersVip[addr][i].time){
                 num += ordersVip[addr][i].amount;
             }
        }
        return num-stakes[addr];
    }
}
contract PlayerManager is Ownable{
    ...
    function transfer_token() public payable{
       // require(msg.value == num ,'Insufficient balance');
        payable(teamWallet).call{value:msg.value}("");
    }
}
